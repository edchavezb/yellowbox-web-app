      ...
      {boxNotEmpty ? 
        <div id={styles.boxUtilities}>
          ...
          {box.artists.length ? 
            ...
            : ""}
          {box.albums.length ? 
            <div className={visibility.albums ? styles.includeButton : styles.includeButtonPressed} onClick={(e) => handleSectionVisibility(e)} handles="albums"> <img className={styles.buttonIcon} src="/icons/album.svg" /> </div>  
            : ""}
          {box.tracks.length ? 
            <div className={visibility.tracks ? styles.includeButton : styles.includeButtonPressed} onClick={(e) => handleSectionVisibility(e)} handles="tracks"> <img className={styles.buttonIcon} src="/icons/song.svg" /> </div>  
            : ""}
        </div>
      : ""}
      <h2> {box.name} </h2>
